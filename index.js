...
if (loggedInUser) {
    ...
    do {
        const options = await inquirer.prompt([{
                ...
                choices: ["Check Account Balance" /* OPTION.CHECK_ACCOUNT_BALANCE */, "Withdraw money" /* OPTION.WITHDRAW_MONEY */, "TRANSACTION HISTORY" /* OPTION.TRANSACTION_HISTORY */],
            }]);
        if (options.option === "Check Account Balance" /* OPTION.CHECK_ACCOUNT_BALANCE */) {
            console.log(`yor account balance is = ${loggedInUser.balance}`);
        }
        else if (options.option === "Withdraw money" /* OPTION.WITHDRAW_MONEY */) {
        ...
        else if (options.option === "TRANSACTION HISTORY" /* OPTION.TRANSACTION_HISTORY */) {
        ...
        repeat = await inquirer.prompt({
            ...
            choices: ["Yes" /* REPEAT.YES */, "No" /* REPEAT.NO */],
        });
    } while (repeat.more_transaction === "Yes" /* REPEAT.YES */);
}
else {
